-- Reconstructed from the live Field Photo Prep Supabase catalog.
-- This migration version is already applied to project vtyiktvqhbgabawotkrj.
-- Keep this file aligned with the deployed schema; do not re-number applied history.
...
create or replace function private.fpp_lock_active_organization(
...
create or replace function private.fpp_require_active_owner(
    p_organization_id uuid)
returns uuid
language plpgsql
security definer
...
create or replace function private.fpp_write_admin_audit(
    p_organization_id uuid,
...
create or replace function public.fpp_admin_list_members(
    p_organization_id uuid)
returns table(
    membership_id uuid,
    user_id uuid,
...
create or replace function public.fpp_admin_list_invitations(
    p_organization_id uuid)
returns table(
    invitation_id uuid,
    email text,
...
create or replace function public.fpp_admin_prepare_invitation(
    p_organization_id uuid,
    p_email text,
...
begin
    ...
    if v_existing_membership_id is not null then
        perform private.fpp_write_admin_audit(
            p_organization_id, v_actor, v_existing_membership_id, null,
            'INVITATION_PREPARED', 'REJECTED');
    ...
    if found then
        if v_invitation.intended_role <> p_intended_role then
            perform private.fpp_write_admin_audit(
                p_organization_id, v_actor, null, v_invitation.id,
                'INVITATION_PREPARED', 'REJECTED');
            return jsonb_build_object(
        ...
        perform private.fpp_write_admin_audit(
            p_organization_id, v_actor, null, v_invitation.id,
            'INVITATION_PREPARED', 'IDEMPOTENT');
    ...
    insert into public.fpp_invitations (
        organization_id, email, intended_role, status,
        invited_by_user_id, expires_at, delivery_status)
    values (
        p_organization_id, v_email, p_intended_role, 'PENDING',
        v_actor, now() + interval '7 days', 'NOT_SENT')
    returning * into v_invitation;

    perform private.fpp_write_admin_audit(
        p_organization_id, v_actor, null, v_invitation.id,
        'INVITATION_PREPARED', 'SUCCEEDED');
...
create or replace function public.fpp_admin_update_invitation_role(
    p_organization_id uuid,
    p_invitation_id uuid,
    p_intended_role text)
returns jsonb
...
begin
    ...
    if v_invitation.intended_role = p_intended_role then
        perform private.fpp_write_admin_audit(
            p_organization_id, v_actor, null, v_invitation.id,
            'INVITATION_ROLE_CHANGED', 'IDEMPOTENT');
        return jsonb_build_object(
            'outcome', 'UNCHANGED',
            'invitation_id', v_invitation.id,
            'intended_role', v_invitation.intended_role);
    ...
    perform private.fpp_write_admin_audit(
        p_organization_id, v_actor, null, v_invitation.id,
        'INVITATION_ROLE_CHANGED', 'SUCCEEDED');
...
create or replace function public.fpp_admin_cancel_invitation(
    p_organization_id uuid,
    p_invitation_id uuid)
returns jsonb
language plpgsql
...
begin
    ...
    if v_invitation.status = 'CANCELLED' then
        perform private.fpp_write_admin_audit(
            p_organization_id, v_actor, null, v_invitation.id,
            'INVITATION_CANCELLED', 'IDEMPOTENT');
    ...
    perform private.fpp_write_admin_audit(
        p_organization_id, v_actor, null, v_invitation.id,
        'INVITATION_CANCELLED', 'SUCCEEDED');

    return jsonb_build_object('outcome', 'CANCELLED', 'invitation_id', v_invitation.id);
end;
$$;

create or replace function public.fpp_admin_record_invitation_delivery(
    p_organization_id uuid,
    p_invitation_id uuid,
    p_succeeded boolean)
returns jsonb
...
create or replace function public.fpp_admin_change_membership_role(
    p_organization_id uuid,
...
begin
    ...
    if v_target.role = p_role then
        perform private.fpp_write_admin_audit(
            p_organization_id, v_actor, v_target.id, null,
            'MEMBERSHIP_ROLE_CHANGED', 'IDEMPOTENT');
        return jsonb_build_object(
            'outcome', 'UNCHANGED',
    ...
    if v_target.status = 'ACTIVE'
       ...
       and p_role = 'MEMBER' then
        ...
        if v_owner_count <= 1 then
            perform private.fpp_write_admin_audit(
                p_organization_id, v_actor, v_target.id, null,
                'MEMBERSHIP_ROLE_CHANGED', 'REJECTED');
            return jsonb_build_object(
    ...
    perform private.fpp_write_admin_audit(
        p_organization_id, v_actor, v_target.id, null,
        'MEMBERSHIP_ROLE_CHANGED', 'SUCCEEDED');
...
create or replace function public.fpp_admin_revoke_membership(
    p_organization_id uuid,
    p_membership_id uuid)
returns jsonb
language plpgsql
...
begin
    ...
    perform private.fpp_write_admin_audit(
        p_organization_id, v_actor, v_target.id, null,
        'MEMBERSHIP_REVOKED', 'SUCCEEDED');

    return jsonb_build_object('outcome', 'REVOKED', 'membership_id', v_target.id);
end;
$$;

create or replace function public.fpp_admin_reactivate_membership(
    p_organization_id uuid,
    p_membership_id uuid)
returns jsonb
language plpgsql
...
begin
    ...
    if v_target.status = 'ACTIVE' then
        perform private.fpp_write_admin_audit(
            p_organization_id, v_actor, v_target.id, null,
            'MEMBERSHIP_REACTIVATED', 'IDEMPOTENT');
    ...
    perform private.fpp_write_admin_audit(
        p_organization_id, v_actor, v_target.id, null,
        'MEMBERSHIP_REACTIVATED', 'SUCCEEDED');

    return jsonb_build_object('outcome', 'ACTIVE', 'membership_id', v_target.id);
end;
$$;

create or replace function public.fpp_accept_invitation(
    p_invitation_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
    ...
    if v_membership.id is null then
        ...
        values (
            v_organization_id, v_actor, v_invitation.intended_role, 'ACTIVE')
